    ...
    <>
      <div className={props.className}>
        <div className="flex flex-row gap-[69px] items-start justify-start w-auto">
          <button onClick={() => props.handleClick(0)} className={`md:text-3xl sm:text-[28px] text-[32px] ${btnColor} w-auto transition-colors duration-300`}
              size="txtKirangHaerangRegular32"
              >{props?.homebuttontext}</button>
          <button onClick={() => props.handleClick(1)} className={`md:text-3xl sm:text-[28px] text-[32px] ${btnColor} w-auto transition-colors duration-300`}
              size="txtKirangHaerangRegular32"
